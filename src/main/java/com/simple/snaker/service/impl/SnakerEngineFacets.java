...
public class SnakerEngineFacets {
	...
	public void initFlows() {
        engine.process().deploy(StreamHelper.getStreamFromClasspath("/WEB-INF/flows/loan.snaker"));
	}
	
	public String initFlows2(){
		//每次初始化流程定义 都会version+1  所以启动一次之后 存数据库中之后 把id固定写死就好 就在该流程上跑 在该流程上定义通的实例
		String processId = engine.process().deploy(StreamHelper.getStreamFromClasspath("flows/loan.snaker"));
		return processId;
	}
	...
	public void closeProcess(final String orderId){
		iOrderService.terminate(orderId, engine.ADMIN);
	}
}
